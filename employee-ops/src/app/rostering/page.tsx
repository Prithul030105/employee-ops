...
export default function AgenticRoster() {
  ...
  const filteredAgents = agents.filter(agent => {
    ...
    const matchesFilter =
      filter === "All" || agent.unit.toLowerCase() === filter.toLowerCase();
  ...
  const handleAutoAssign = (targetLat?: number, targetLng?: number, unit?: string) => {
    ...
    if (targetLat !== undefined && targetLng !== undefined) {
      ...
      setAgents(prev =>
        prev.map(a =>
          a.id === selected.id
            ? { ...a, location: { lat: targetLat, lng: targetLng } }
            : a
        )
      );
    } else {
    ...
    <div className="p-4 md:p-8 xl:p-12 max-w-7xl mx-auto bg-[#f6f4ef] min-h-screen space-y-10">
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-2">
        ...
        <p className="text-slate-500 font-medium">
          Live deployment & dynamic assignment
        </p>
      </div>
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border space-y-6">
        ...
        <div className="flex flex-col md:flex-row gap-4 md:items-end">
          <input
            type="text"
            placeholder="Target Lat (optional)"
          ...
          <input
            type="text"
            placeholder="Target Lng (optional)"
            className="px-4 py-2 border rounded-lg w-full"
          ...
          <button
            ...
            onClick={() =>
              handleAutoAssign(
                manual.lat ? Number(manual.lat) : undefined,
                manual.lng ? Number(manual.lng) : undefined,
                manual.unit || undefined
              )
            }
          >
            Auto Assign
        ...
        <div className="flex flex-col md:flex-row gap-4 md:items-end">
          <select
            className="px-4 py-2 border rounded-lg w-full"
            value={manual.agentId}
            onChange={e =>
              setManual({ ...manual, agentId: Number(e.target.value) })
            }
          >
          ...
          <button
            className="px-4 py-2 bg-slate-600 text-white rounded-lg font-semibold hover:bg-slate-700 transition w-full md:w-auto"
      ...
      <div className="flex flex-col md:flex-row gap-4">
        <input
          type="text"
          placeholder="Search by agent or role..."
          value={search}
          onChange={e => setSearch(e.target.value)}
          className="flex-1 px-6 py-3 rounded-2xl border bg-white shadow-sm"
        />
        <select
          value={filter}
          onChange={e => setFilter(e.target.value)}
          className="px-6 py-3 rounded-2xl border bg-white shadow-sm"
        >
            ...
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  📍 {agent.location.lat.toFixed(4)},{" "}
                  {agent.location.lng.toFixed(4)}
                </p>
              ...
              <span
                className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest ${getStatusStyle(
                  agent.status
                )}`}
              >
                {agent.status}
