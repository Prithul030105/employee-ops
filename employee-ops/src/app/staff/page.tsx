...
export default function StaffDirectory() {
  ...
  const filteredStaff = staff.filter((member) => {
    ...
    const matchesFilter =
      filter === "All" || member.department.toLowerCase() === filter.toLowerCase();
  ...
  const markLeft = (id: number) => {
    setStaff(prev => prev.map(s => s.id === id ? { ...s, status: "Left Company" } : s));
  };

  const saveEdit = (id: number) => {
    setStaff(prev => prev.map(s => s.id === id ? { ...s, ...editStaff } as StaffMember : s));
    setEditingId(null);
    setEditStaff({});
  };
    ...
    <div className="px-4 sm:px-6 md:px-10 py-8 max-w-7xl mx-auto bg-slate-50 min-h-screen space-y-10">
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-6">
        <div>
          ...
          <p className="text-slate-500 font-medium">
            Organization-wide employee registry
          </p>
        </div>
        <button
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <input type="text" placeholder="Full Name" value={newStaff.name}
              ...
              className="px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-slate-300 outline-none" />
            <input type="text" placeholder="Role" value={newStaff.role}
          ...
          <button
            className="mt-4 px-6 py-2 bg-slate-900 text-white rounded-lg hover:bg-slate-800 transition"
      ...
      <div className="flex flex-col md:flex-row gap-4">
        <input
          ...
          className="flex-1 px-6 py-3 rounded-xl border border-slate-200 bg-white shadow-sm focus:ring-2 focus:ring-slate-300 outline-none"
        />
        <select
          value={filter}
          onChange={e => setFilter(e.target.value)}
          className="px-6 py-3 rounded-xl border border-slate-200 bg-white shadow-sm"
        >
          ...
          <div key={member.id}
            ...
            {editingId === member.id ? (
              <div className="space-y-3">
                <input type="text" value={editStaff.name || member.name}
                  onChange={e => setEditStaff(prev => ({ ...prev, name: e.target.value }))}
                  className="px-4 py-2 border border-slate-200 rounded-lg w-full" />
                <input type="text" value={editStaff.role || member.role}
                  onChange={e => setEditStaff(prev => ({ ...prev, role: e.target.value }))}
                  className="px-4 py-2 border border-slate-200 rounded-lg w-full" />
                <input type="email" value={editStaff.email || member.email}
                  onChange={e => setEditStaff(prev => ({ ...prev, email: e.target.value }))}
                  className="px-4 py-2 border border-slate-200 rounded-lg w-full" />
                <div className="flex gap-3 mt-2">
                  <button className="px-4 py-2 bg-slate-900 text-white rounded-lg hover:bg-slate-800 transition"
                    onClick={() => saveEdit(member.id)}>Save</button>
                  <button className="px-4 py-2 bg-slate-200 rounded-lg hover:bg-slate-300 transition"
              ...
              <>
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className={`text-lg font-bold ${member.status === "Left Company" ? "text-slate-400 line-through" : "text-slate-900"}`}>
                      {member.name}
                    </h3>
                    <p className="text-sm text-slate-500 mt-1">{member.role}</p>
                  </div>
                  <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide ${getStatusStyle(member.status)}`}>
                    {member.status}
                  </span>
                </div>
                ...
                <div className="mt-6 flex flex-col gap-2">
                  {member.status !== "Left Company" && (
                    <button
                      className="w-full py-2 rounded-lg border border-red-300 text-red-600 font-medium hover:bg-red-50 transition"
                      onClick={() => markLeft(member.id)}>
                      Mark as Left
                    </button>
                  )}
                  <button
                    className="w-full py-2 rounded-lg border border-slate-200 text-sm font-medium text-slate-600 hover:bg-slate-50 transition"
                    onClick={() => { setEditingId(member.id); setEditStaff(member); }}>
                    Edit Details
                  </button>
                </div>
              </>
